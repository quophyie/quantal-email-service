...
COMMAND="WEB_APP_PORT=${WEB_APP_PORT} docker-compose -f ../docker/compose/docker-compose.yml up ${DB_CONTAINER_NAME} ${BUILD_CONTAINER_COMMAND} DB_PORT=${DB_PORT_EXPOSED_ON_DB_CONTAINER} WEB_APP_PORT=${WEB_APP_PORT} NODE_ENV=${NODE_ENV} docker-compose -f ../docker/compose/docker-compose.yml up ${COMPOSE_UP_OPTS}"
